...
# Specify the directory to extract Maven (with versioned folder directly)
$installDir = "C:\software\Maven\maven-$mavenVersion"

# Extract the Maven zip directly into the versioned directory
Write-Host "Extracting Maven to $installDir"
Expand-Archive -Path $downloadPath -DestinationPath $installDir -Force

# Set Maven Environment Variables
$env:M2_HOME = $installDir
$env:Path = "$env:M2_HOME\bin;$env:Path"
